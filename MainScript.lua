...
assert(not shared.VapeExecuted, "fape is injected genius :skullface:")
...
if not isfile("FapeClient/CustomModules/cachechecked.txt") then
	...
	if isNotCached and not shared.VapeDeveloper then
		displayErrorPopup("Bro u have error in fape :skull: if u have customs click no, else click yes.", {No = function() end, Yes = function()
			for i,v in pairs({"FapeClient/Universal.lua", "FapeClient/MainScript.lua", "FapeClient/GuiLibrary.lua"}) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					delfile(v)
				end 
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/ImNicknamez/FapeClient/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/ImNicknamez/FapeClient/"..saveplaceid.."/"..v2.OnlineProfileName
...
local function loadVape()
	if not shared.VapeIndependent then
		...
		--if shared.VapePrivate then
		if isfile("FapeClientprivCustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("FapeClientprivCustomModules/"..game.PlaceId..".lua"))()
		end	
		--end
	else
